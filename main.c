    ...
    int main() {
        ...
        if (pid == 0) {
            ...
            if (waitpid(pid, NULL, 0) == -1) {
                perror("[MAIN][ERROR] Błąd oczekiwania na proces rejestracja");
            } else {
                printf("[MAIN][DEBUG] Proces rejestracja zakończył się z PID: %d\n", pid);
            }
        } else {
            perror("[MAIN][ERROR] Błąd forka");
        }
        //     if (fork() == 0) {
