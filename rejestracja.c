...
void zarzadz_i_monitoruj_rejestracje() {
    ...
    if (rejestracja_pid == 0) {
        // Proces potomny
        rejestracja(0); // Rejestracja działa w osobnym procesie
        //exit(0);        // Wyjście z procesu potomnego po zakończeniu funkcji rejestracja
    } else if (rejestracja_pid > 0) {
        printf("[Monitorowanie] Rejestracja (okienko 0) uruchomiona w procesie PID: %d\n", rejestracja_pid);
    } else {
        perror("Błąd podczas tworzenia procesu rejestracji");
        exit(1);
    }
